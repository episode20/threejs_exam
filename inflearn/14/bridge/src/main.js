...
spotLight1.shadow.mapSize.width = 2048; // mapSize는 성능에 영향을 미칠 수 있음.
...
// 물리엔진
cm1.world.gravity.set(0, -10, 0); // 중력설정
...
const pillar1 = new Pillar({
  ...
  x: 0,
  y: 5.5,
  z: -glassUnitSize * 12 - glassUnitSize / 2, // 1.2는 유리창 사이즈
});
  ...
  objects.forEach((item) => {
    if (item.cannonBody) {
      if (item.name === "player") {
        ...
        if (fail) item.mesh.position.copy(item.cannonBody.position);
