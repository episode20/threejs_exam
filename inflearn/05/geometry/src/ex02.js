...
/**
 * ----- 주제: Geometry 기본
 *  const constrols = new OrbitControls(camera, renderer.domElement); //canvas
 * 
 *  const material = new THREE.MeshStandardMaterial({
	...
	geometry의 종류는 공식문서에서 확인가능 BoxGeometry, CircleGeometry
 *  */
...
export default function example() {
  ...
  console.log(geometry.attributes.position.array); // x,y,z,x,y,z,x,y,z,...
  const positionArray = geometry.attributes.position.array;
  const randomArray = [];
  for (let i = 0; i < positionArray.length; i += 3) {
    // 정점(vertax) 한개의 x,y,z의 좌표를 랜덤으로 조정
